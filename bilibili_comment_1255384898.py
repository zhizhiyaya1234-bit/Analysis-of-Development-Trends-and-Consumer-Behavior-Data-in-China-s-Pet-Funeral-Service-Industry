...
class BilibiliCommentCrawler:
    ...
    def get_video_info(self, aid: int = 1255384898) -> dict:
        """获取视频信息"""
        try:
    ...
    def get_comments_page(self, oid: int = 1255384898, page: int = 1) -> dict:
        """获取单页评论数据"""
        try:
    ...
    def get_sub_comments(self, root_id: int, oid: int = 1255384898, page: int = 1) -> dict:
        """获取子评论（楼中楼）"""
        try:
    ...
    def parse_comment(self, comment_data: dict, video_id: int = 1255384898) -> dict:
    ...
    def crawl_comments(self, 
        ...
        """
        all_comments = []
        oid = 1255384898  # 视频aid
        
        print("=" * 60)
        print(f"开始爬取B站视频 1255384898 的评论")
        print("=" * 60)
    ...
    def save_to_excel(self, comments: list, filename: str = None):
        ...
        if filename is None:
            filename = f"B站评论数据_video_1255384898_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            df.to_excel(writer, sheet_name='video_1255384898', index=False)
